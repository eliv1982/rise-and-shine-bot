...
Telegram-бот для ежедневных аффирмаций: генерация текста (Yandex GPT), картинок (OpenAI-совместимый API) и озвучка (Yandex SpeechKit). Поддержка голосового ввода темы и стиля, подписка на рассылку.
...
- **Генерация изображений** — стили (реалистичный, природа, космос, мандала и др.), опциональное описание голосом или текстом.
- **Озвучка** — Yandex SpeechKit TTS, паузы между аффирмациями (ffmpeg).
- **Ежедневная рассылка** — настройка времени и сферы для подписчиков (APScheduler).
...
- **[DEPLOY_DOCKERHUB.md](DEPLOY_DOCKERHUB.md)** — деплой через образ на Docker Hub (без сборки на сервере).
